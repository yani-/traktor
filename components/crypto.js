...
export default class Crypto {
    ...
    extractIv(base64EncryptedString) {
        return Buffer.from(base64EncryptedString, 'base64')
            .slice(0, IV_LENGTH)
    }

    extractEncryptedText(base64EncryptedString) {
        return Buffer.from(base64EncryptedString, 'base64')
            .slice(IV_LENGTH)
            .toString('base64');
    }
    ...
    async decryptFile(decryptionKey, fileContent) {
        ...
        for(let i = 0; i < chunksNumber; i++ ) {
            ...
            const res = this.decrypt(encryptedText, decryptionKey, IV);
